...
def E_box_1D_analytic_AU(n_states, L, mass=m_e):
    n = np.arange(1, n_states + 1)
    energies = (n**2 * np.pi**2) / (2.0 * mass * L**2)
    return energies

def E_qho_1D_analytic_AU(n_states, L, mass=m_e, k=1.0):
...
def E_box_2D_analytic_AU(n_states, L, mass=m_e):
...
def E_qho_2D_analytic_AU(n_states, L, mass=m_e, k=1.0):
...
def E_box_3D_analytic_AU(n_states, L, mass=m_e):
    base_energy = (np.pi**2) / (2.0 * mass * L**2)
    energies = []
...
def E_qho_3D_analytic_AU(n_states, L, mass=m_e, k=1.0):
    # L is ignored for analytic formula
    base_energy = np.sqrt(k / mass)
    energies = []
    N = 0 # N = nx + ny + nz
