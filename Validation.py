...
class Validate(Hamiltonian):
    ...
    """
    def __init__(self, test_case, N, L, num_states, mass='electron', **kwargs):
        """ 
        Args:
        ...
        if test_case == 'box_1d':
            ndim = 1
            potential_func = V_box_1D_AU
            self.analytic_func = E_box_1D_analytic_AU
            self.test_case_name = "1D Particle in a Box"
    
        elif test_case == 'qho_1d':
            ndim = 1
            potential_func = lambda x, L: V_qho_1D_AU(x, L, k=self.k)
            self.analytic_func = lambda n, L, m: E_qho_1D_analytic_AU(n, L, m, k=self.k)
            self.test_case_name = "1D Quantum Harmonic Oscillator"
    
        elif test_case == 'box_2d':
            ndim = 2
            potential_func = V_box_2D_AU
            self.analytic_func = E_box_2D_analytic_AU
            self.test_case_name = "2D Particle in a Box"
    
        elif test_case == 'qho_2d':
            ndim = 2
            potential_func = lambda x, y, L: V_qho_2D_AU(x, y, L, k=self.k)
            self.analytic_func = lambda n, L, m: E_qho_2D_analytic_AU(n, L, m, k=self.k)
            self.test_case_name = "2D Quantum Harmonic Oscillator"
    
        elif test_case == 'box_3d':
            ndim = 3
            potential_func = V_box_3D_AU
            self.analytic_func = E_box_3D_analytic_AU
            self.test_case_name = "3D Particle in a Box"
    
        elif test_case == 'qho_3d':
            ndim = 3
            potential_func = lambda x, y, z, L: V_qho_3D_AU(x, y, z, L, k=self.k)
            self.analytic_func = lambda n, L, m: E_qho_3D_analytic_AU(n, L, m, k=self.k)
