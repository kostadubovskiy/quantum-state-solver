...
class Split_Operator:
    def __init__(
        ...
        """
        N, L: grid size and length (same as Hamiltonian)
        potential_func(x, L, **pot_kwargs): user potential
        mass: 1.0 for electron in atomic units
        dt: time step
        bc: should be 'periodic' for FFT; absorbing mask added separately
        """
        self.N = N
        self.L = L
        self.dx = L / (N - 1)
        ...
        # Optional absorbing mask
        self.mask = np.ones_like(self.x)

    def absorbing_boundary(self, start_frac=0.8, p=6):
        """Adds a smooth mask (0 < mask <= 1) for absorbing outgoing waves."""
...
# ============================================================================
# Time Evolution via Eigenstate Expansion (works for 1D, 2D, 3D)
# ============================================================================
